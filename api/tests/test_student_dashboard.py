...
def test_dashboard(login, query_dashboard, user_employee, user_student, job_posting_objects,
                   branch_objects, student_challenge_objects, company_challenge_objects):
    ...
    assert len(challenges) == len(student_challenge_objects)

    latest_challenges = dashboard.get('latestChallenges')
    assert len(latest_challenges) == len(company_challenge_objects) - 1    # 2x public, 1x draft
