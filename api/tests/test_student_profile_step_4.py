...
def test_step_4(login, user_student, student_step_4, skill_objects, language_objects,
                language_level_objects):
    user_student.student.profile_step = 4
    user_student.student.save()
    login(user_student)
    data, errors = student_step_4(
        user_student,
        skill_objects,
        (
...
@pytest.mark.django_db
def test_step_4_without_login(user_student, student_step_4, skill_objects, language_objects,
                              language_level_objects):
    data, errors = student_step_4(AnonymousUser(), skill_objects,
                                  ((language_objects[0], language_level_objects[0]), ), None, None,
                                  '')
...
def test_step_4_as_company(login, user_employee, student_step_4, skill_objects, language_objects,
                           language_level_objects):
    login(user_employee)
    data, errors = student_step_4(user_employee, skill_objects,
                                  ((language_objects[0], language_level_objects[0]), ), None, None,
                                  '')
    assert errors is None
    assert data is not None
...
@pytest.mark.django_db
def test_step_4_update_delete_languages(login, user_student, student_step_4, skill_objects,
