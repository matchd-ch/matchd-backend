...
def test_step_2_as_company(user_employee, login, project_posting_step_2,
                           company_project_posting_object):
    _test_step_2(user_employee, user_employee.company, None, company_project_posting_object, login,
                 project_posting_step_2)


@pytest.mark.django_db
def test_step_2_as_student(user_student, login, project_posting_step_2,
                           company_project_posting_object):
    _test_step_2(user_student, None, user_student.student, company_project_posting_object, login,
                 project_posting_step_2)


def _test_step_2(user, company, student, company_project_posting_object, login,
                 project_posting_step_2):
    login(user)
    ...
    data, errors = project_posting_step_2(user, company_project_posting_object.id, '03.2021',
    ...
    project_posting = ProjectPosting.objects.get(
        pk=data.get('projectPostingStep2').get('projectPostingId'))
...
@pytest.mark.django_db
def test_step_2_with_invalid_data(user_employee, login, project_posting_step_2,
                                  company_project_posting_object):
    login(user_employee)
    data, errors = project_posting_step_2(user_employee, company_project_posting_object.id,
                                          '78.2021', 'invalid-url')
    assert errors is None
    assert data is not None
    assert data.get('projectPostingStep2') is not None
...
@pytest.mark.django_db
def test_step_2_as_employee_from_another_company(user_employee, user_employee_2,
...
@pytest.mark.django_db
def test_step_2_as_student_with_project_of_company(user_employee, user_student,
                                                   company_project_posting_object, login,
                                                   project_posting_step_2):
    login(user_employee)
    company_project_posting_object.form_step = 2
    ...
    company_project_posting_object.student = None
    company_project_posting_object.save()
    data, errors = project_posting_step_2(user_student, company_project_posting_object.id,
                                          '03.2021', 'www.project-posting.lo')
    assert errors is None
    assert data is not None
    assert data.get('projectPostingStep2') is not None
