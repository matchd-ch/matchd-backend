...
def test_query(query_topics, topic_objects):
    ...
    assert_node_id(edges[0].get('node'), f'Topic:{topic_objects[0].id}')
    assert_node_id(edges[1].get('node'), f'Topic:{topic_objects[1].id}')
...
def test_node_query(query_topic_node, topic_objects):
    data, errors = query_topic_node(AnonymousUser(), f'Topic:{topic_objects[0].id}')
    ...
    assert_node_id(node, f'Topic:{topic_objects[0].id}')
