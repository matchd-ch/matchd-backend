...
def test_query(query_cultural_fits, cultural_fit_objects):
    ...
    assert_node_id(edges[0].get('node'), 'CulturalFit', cultural_fit_objects[0].id)
    assert_node_id(edges[1].get('node'), 'CulturalFit', cultural_fit_objects[1].id)
...
def test_node_query(query_cultural_fit_node, cultural_fit_objects):
    data, errors = query_cultural_fit_node(AnonymousUser(), cultural_fit_objects[0].id)
    ...
    assert_node_id(node, 'CulturalFit', cultural_fit_objects[0].id)
