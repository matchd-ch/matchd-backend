...
def test_query(query_keywords, keyword_objects):
    ...
    assert_node_id(edges[0].get('node'), 'Keyword', keyword_objects[1].id)
    assert_node_id(edges[1].get('node'), 'Keyword', keyword_objects[0].id)
...
def test_node_query(query_keyword_node, keyword_objects):
    data, errors = query_keyword_node(AnonymousUser(), keyword_objects[1].id)
    ...
    assert_node_id(node, 'Keyword', keyword_objects[1].id)
