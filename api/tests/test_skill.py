...
def test_query(query_skills, skill_objects):
    ...
    assert_node_id(edges[0].get('node'), f'Skill:{skill_objects[1].id}')
    assert_node_id(edges[1].get('node'), f'Skill:{skill_objects[2].id}')
    assert_node_id(edges[2].get('node'), f'Skill:{skill_objects[0].id}')
    assert_node_id(edges[3].get('node'), f'Skill:{skill_objects[3].id}')
...
def test_node_query(query_skill_node, skill_objects):
    data, errors = query_skill_node(AnonymousUser(), f'Skill:{skill_objects[0].id}')
    ...
    assert_node_id(node, f'Skill:{skill_objects[0].id}')
