...
def test_query(query_branches, branch_objects):
    ...
    assert_node_id(edges[0].get('node'), f'Branch:{branch_objects[1].id}')
    assert_node_id(edges[1].get('node'), f'Branch:{branch_objects[0].id}')
...
def test_node_query(query_branch_node, branch_objects):
    data, errors = query_branch_node(AnonymousUser(), f'Branch:{branch_objects[1].id}')
    ...
    assert_node_id(node, f'Branch:{branch_objects[1].id}')
