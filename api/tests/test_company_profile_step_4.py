...
def test_step_4(login, user_employee, company_step_4, soft_skill_objects, cultural_fit_objects):
    user_employee.company.profile_step = 4
    user_employee.company.save()
    login(user_employee)
    data, errors = company_step_4(user_employee, soft_skill_objects[:6], cultural_fit_objects[:6])
    assert errors is None
    assert data is not None
    assert data.get('companyProfileStep4') is not None
    assert data.get('companyProfileStep4').get('success')
...
def test_step_4_without_login(user_employee, company_step_4, soft_skill_objects, cultural_fit_objects):
    ...
    assert data is not None
    assert data.get('companyProfileStep4') is None
...
def test_step_4_as_student(login, user_student, company_step_4, soft_skill_objects, cultural_fit_objects):
    ...
    errors = data.get('companyProfileStep4').get('errors')
    assert errors is not None
...
def test_step_4_invalid_step(login, user_employee, company_step_4, soft_skill_objects, cultural_fit_objects):
    ...
    assert data is not None
    assert data.get('companyProfileStep4') is not None
    assert data.get('companyProfileStep4').get('success') is False
...
@pytest.mark.django_db
def test_step_4_invalid_data(login, user_employee, company_step_4):
    user_employee.company.profile_step = 4
    user_employee.company.save()
    login(user_employee)
    data, errors = company_step_4(user_employee, [SoftSkill(id=1337)], [CulturalFit(id=1337)])
    assert errors is None
    assert data is not None
    assert data.get('companyProfileStep4') is not None
    assert data.get('companyProfileStep4').get('success') is False
...
def test_step_4_too_few_soft_skills_and_cultural_fits(login, user_employee, company_step_4, soft_skill_objects,
    ...
    user_employee.company.save()
    login(user_employee)
    data, errors = company_step_4(user_employee, soft_skill_objects[:5], cultural_fit_objects[:5])
    assert errors is None
    assert data is not None
    assert data.get('companyProfileStep4') is not None
    assert data.get('companyProfileStep4').get('success') is False
...
def test_step_4_too_many_soft_skills_and_cultural_fits(login, user_employee, company_step_4, soft_skill_objects,
    ...
    user_employee.company.save()
    login(user_employee)
    data, errors = company_step_4(user_employee, soft_skill_objects[:7], cultural_fit_objects[:7])
    assert errors is None
    assert data is not None
    assert data.get('companyProfileStep4') is not None
    assert data.get('companyProfileStep4').get('success') is False
