...
def test_query(query_benefits, benefit_objects):
    ...
    assert_node_id(edges[0].get('node'), 'Benefit', benefit_objects[1].id)
    assert_node_id(edges[1].get('node'), 'Benefit', benefit_objects[0].id)
...
def test_node_query(query_benefit_node, benefit_objects):
    data, errors = query_benefit_node(AnonymousUser(), benefit_objects[0].id)
    ...
    assert_node_id(node, 'Benefit', benefit_objects[0].id)
