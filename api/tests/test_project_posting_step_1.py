...
def test_step_1_as_company(user_employee, login, project_posting_step_1, topic_objects,
                           project_type_objects, keyword_objects):
    _test_step_1(user_employee, login, project_posting_step_1, topic_objects, project_type_objects,
                 keyword_objects)


@pytest.mark.django_db
def test_step_1_as_student(user_student, login, project_posting_step_1, topic_objects,
                           project_type_objects, keyword_objects):
    _test_step_1(user_student, login, project_posting_step_1, topic_objects, project_type_objects,
                 keyword_objects)
...
def _test_step_1(user, login, project_posting_step_1, topic_objects, project_type_objects,
                 keyword_objects):
    login(user)
    data, errors = project_posting_step_1(user, 'title', 'description', 'additional information',
    ...
    project_posting_slug = ProjectPosting.objects.get(
        slug=data.get('projectPostingStep1').get('slug'))
    project_posting = ProjectPosting.objects.get(
        pk=data.get('projectPostingStep1').get('projectPostingId'))
...
def test_step_1_without_login(project_posting_step_1, topic_objects, project_type_objects,
                              keyword_objects):
    data, errors = project_posting_step_1(AnonymousUser(), 'title', 'description',
...
def test_step_1_with_invalid_data(user_employee, login, project_posting_step_1):
    login(user_employee)
    data, errors = project_posting_step_1(user_employee, '', '', '', Topic(id=1337),
