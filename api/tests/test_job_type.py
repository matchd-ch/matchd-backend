...
def test_query(query_job_types, job_type_objects):
    ...
    assert_node_id(edges[0].get('node'), f'JobType:{job_type_objects[1].id}')
    assert_node_id(edges[1].get('node'), f'JobType:{job_type_objects[2].id}')
    assert_node_id(edges[2].get('node'), f'JobType:{job_type_objects[0].id}')
    assert_node_id(edges[3].get('node'), f'JobType:{job_type_objects[3].id}')
...
def test_node_query(query_job_type_node, job_type_objects):
    data, errors = query_job_type_node(AnonymousUser(), f'JobType:{job_type_objects[3].id}')
    ...
    assert_node_id(node, f'JobType:{job_type_objects[3].id}')
