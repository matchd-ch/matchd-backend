...
def test_step_3_as_company(user_employee, company_project_posting_object, login,
                           project_posting_step_3):
    _test_step_3(user_employee, user_employee.employee, user_employee.company, None,
                 company_project_posting_object, login, project_posting_step_3)


@pytest.mark.django_db
def test_step_3_as_student(user_student, company_project_posting_object, login,
                           project_posting_step_3):
    _test_step_3(user_student, None, None, user_student.student, company_project_posting_object,
                 login, project_posting_step_3)


def _test_step_3(user, employee, company, student, company_project_posting_object, login,
                 project_posting_step_3):
    login(user)
    ...
    data, errors = project_posting_step_3(user, company_project_posting_object.id,
    ...
    project_posting_slug = ProjectPosting.objects.get(
        slug=data.get('projectPostingStep3').get('slug'))
    project_posting = ProjectPosting.objects.get(
        pk=data.get('projectPostingStep3').get('projectPostingId'))
...
def test_step_3_with_invalid_job_posting_id(user_employee, login, project_posting_step_3):
    login(user_employee)
    data, errors = project_posting_step_3(user_employee, 1337, ProjectPostingState.PUBLIC,
...
@pytest.mark.django_db
def test_step_3_without_login(user_employee, company_project_posting_object,
                              project_posting_step_3):
...
@pytest.mark.django_db
def test_step_3_with_invalid_step(user_employee, company_project_posting_object, login,
                                  project_posting_step_3):
    login(user_employee)
    company_project_posting_object.form_step = 1
    company_project_posting_object.save()
    data, errors = project_posting_step_3(user_employee, company_project_posting_object.id,
                                          ProjectPostingState.PUBLIC, user_employee.employee)
    assert errors is None
    assert data is not None
    assert data.get('projectPostingStep3') is not None
...
@pytest.mark.django_db
def test_step_3_as_employee_from_another_company(user_employee, user_employee_2,
...
@pytest.mark.django_db
def test_step_3_as_student_with_project_of_company(user_employee, user_student,
                                                   company_project_posting_object, login,
                                                   project_posting_step_3):
    login(user_employee)
    company_project_posting_object.form_step = 3
    ...
    company_project_posting_object.student = None
    company_project_posting_object.save()
    data, errors = project_posting_step_3(user_student, company_project_posting_object.id,
                                          ProjectPostingState.PUBLIC, user_employee.employee)
    assert errors is None
    assert data is not None
    assert data.get('projectPostingStep3') is not None
...
@pytest.mark.django_db
def test_step_3_as_company_with_project_of_student(user_employee, user_student,
                                                   company_project_posting_object, login,
                                                   project_posting_step_3):
    login(user_employee)
    company_project_posting_object.form_step = 3
