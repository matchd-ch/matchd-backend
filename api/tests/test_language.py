...
def test_query(query_languages, language_objects):
    ...
    assert_node_id(edges[0].get('node'), f'Language:{language_objects[1].id}')
    assert_node_id(edges[1].get('node'), f'Language:{language_objects[2].id}')
    assert_node_id(edges[2].get('node'), f'Language:{language_objects[3].id}')
    assert_node_id(edges[3].get('node'), f'Language:{language_objects[0].id}')
...
def test_shortlist_query(query_languages_shortlist, language_shortlist_objects):
    ...
    assert_node_id(edges[0].get('node'), f'Language:{language_shortlist_objects[0].id}')
    assert_node_id(edges[1].get('node'), f'Language:{language_shortlist_objects[1].id}')
...
def test_node_query(query_language_node, language_objects):
    data, errors = query_language_node(AnonymousUser(), f'Language:{language_objects[1].id}')
    ...
    assert_node_id(node, f'Language:{language_objects[1].id}')
