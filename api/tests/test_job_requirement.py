...
def test_query(query_job_requirements, job_requirement_objects):
    ...
    assert_node_id(edges[0].get('node'), 'JobRequirement', job_requirement_objects[1].id)
    assert_node_id(edges[1].get('node'), 'JobRequirement', job_requirement_objects[0].id)
...
def test_node_query(query_job_requirement_node, job_requirement_objects):
    data, errors = query_job_requirement_node(AnonymousUser(), job_requirement_objects[1].id)
    ...
    assert_node_id(node, 'JobRequirement', job_requirement_objects[1].id)
