...
def test_query(query_soft_skills, soft_skill_objects):
    ...
    assert_node_id(edges[0].get('node'), f'SoftSkill:{soft_skill_objects[0].id}')
    assert_node_id(edges[1].get('node'), f'SoftSkill:{soft_skill_objects[1].id}')
...
def test_node_query(query_soft_skill_node, soft_skill_objects):
    data, errors = query_soft_skill_node(AnonymousUser(), f'SoftSkill:{soft_skill_objects[0].id}')
    ...
    assert_node_id(node, f'SoftSkill:{soft_skill_objects[0].id}')
