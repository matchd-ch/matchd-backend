...
def test_query(query_faq_categories, faq_category_objects):
    ...
    assert_node_id(edges[0].get('node'), f'FAQCategory:{faq_category_objects[0].id}')
    assert_node_id(edges[1].get('node'), f'FAQCategory:{faq_category_objects[1].id}')
...
def test_node_query(query_faq_category_node, faq_category_objects):
    data, errors = query_faq_category_node(AnonymousUser(), f'FAQCategory:{faq_category_objects[0].id}')
    ...
    assert_node_id(node, f'FAQCategory:{faq_category_objects[0].id}')
