...
def test_query(query_project_types, project_type_objects):
    ...
    assert_node_id(edges[0].get('node'), f'ProjectType:{project_type_objects[0].id}')
    assert_node_id(edges[1].get('node'), f'ProjectType:{project_type_objects[1].id}')
...
def test_node_query(query_project_type_node, project_type_objects):
    data, errors = query_project_type_node(AnonymousUser(), f'ProjectType:{project_type_objects[0].id}')
    ...
    assert_node_id(node, f'ProjectType:{project_type_objects[0].id}')
