...
class StudentProfileEmploymentDateForm(forms.Form):
...
class StudentProfileEmploymentDateRangeForm(forms.Form):
...
def process_job_type_form(profile, data):
    ...
    if job_type.mode == DateMode.DATE_RANGE:
        form = StudentProfileEmploymentDateRangeForm(data)
        form.full_clean()
        if form.is_valid():
            # update profile
        ...
        form = StudentProfileEmploymentDateForm(data)
        form.full_clean()
        if form.is_valid():
            # update profile
            cleaned_data = form.cleaned_data
...
def process_student_employment_form(user, data):
    ...
    form = StudentProfileEmploymentForm(data)
    form.full_clean()
