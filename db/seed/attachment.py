...
class Attachment(BaseSeed):
    ...
    content_types = {
        ...
        'challenge': ContentType.objects.get(app_label='db', model='challenge'),
    }
    ...
    def _create_attachment(self, fixtures, company_or_student_challenge, data, user,
                           content_type_key):
        ...
        relative_path = os.path.join(content_type_key, str(company_or_student_challenge.id),
        ...
        if attachment_key in (AttachmentKey.COMPANY_AVATAR, AttachmentKey.STUDENT_AVATAR):
            existing = AttachmentModel.objects.filter(
                key=attachment_key,
                object_id=company_or_student_challenge.id,
    ...
    def _create_or_update_challenge(self, challenge):
        content_type = ContentType.objects.get(app_label='db', model='challenge')
        images = AttachmentModel.objects.filter(key=AttachmentKey.CHALLENGE_IMAGES,
                                                content_type=content_type,
                                                object_id=challenge.id)

        documents = AttachmentModel.objects.filter(key=AttachmentKey.CHALLENGE_DOCUMENTS,
                                                   content_type=content_type,
                                                   object_id=challenge.id)

        user = None
        if challenge.student is not None:
            user = challenge.student.user
        if challenge.employee is not None:
            user = challenge.employee.user

        if len(images) == 0 and challenge.id % 2 == 0:
            for i in range(1, self.rand.number()):
                data = {
                    'file': f'moods/{self.rand.mood()}',
                    'type': 'db.image',
                    'key': 'challenge_images',
                }
                self._create_attachment(self.company_fixtures, challenge, data, user, 'challenge')
        if len(documents) == 0 and challenge.id % 2 == 0:
            for i in range(1, 3):
                data = {
                    'file': f'documents/doc-{i}.pdf',
                    'type': 'db.file',
                    'key': 'challenge_documents',
                }
                self._create_attachment(self.student_fixtures, challenge, data, user, 'challenge')

    def create_or_update(self, data, *args, **kwargs):
        challenges = kwargs.get('challenges')
        if challenges is not None:
            for challenge in challenges:
                self._create_or_update_challenge(challenge)
            return
        if data is None:
            return
